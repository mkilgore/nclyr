...
static int handle_long_arg(int parser_argc, char **parser_argv, const struct arg *args, const char *cur) {
  int i;
...
void display_help_text(const char *prog, const char *arg_str, const char *arg_desc_str, const struct arg *args)
{
  const struct arg *a;
  printf("Usage: %s %s \n"
         "\n"
         "%s"
         "Flags:\n", prog, arg_str, arg_desc_str);
